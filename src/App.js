...
const REPORTS_TO_URGENT = 2; // number of "bin full" reports before urgent WA alert
...
const DEFAULT_STATE = {
  ...
  history:  [],
  alerts:   [],   // { id, binType, binLabel, reports: [name,...], ts }
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:"24px" }}>
      <div style={{ background:T.bgCard, borderRadius:"20px", padding:"28px 24px", width:"100%", maxWidth:"320px", boxShadow:"0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        <input
          ...
          autoFocus
          style={{ width:"100%", background:T.bgInput, border:`2px solid ${error?"#ff3b30":T.border}`, borderRadius:"12px", padding:"12px 16px", color:T.text, fontSize:"20px", textAlign:"center", letterSpacing:"8px", marginBottom:"12px", transition:"border 0.2s" }}
        />
...
export default function BinRota() {
  ...
  function toggleTheme() {
    setIsDark((v) => {
      localStorage.setItem("binrota-theme", !v ? "dark" : "light");
      return !v;
    });
  }
  ...
  // Urgent alert = any bin with 2+ reports
  ...
  function reportFull(binTypeId, reporterName) {
    ...
    if (existing) {
      if (existing.reports && existing.reports.includes(name)) return; // already reported by this name
      const updated = { ...existing, reports: [...(existing.reports || []), name] };
      saveState({ alerts: alerts.map((a) => a.binType === binTypeId ? updated : a) });
    } else {
      saveState({ alerts: [...alerts, { id: Date.now(), binType: binTypeId, binLabel: bin.label, reports: [name], ts: Date.now() }] });
    }
  }

  function dismissAlert(binTypeId) {
    saveState({ alerts: alerts.filter((a) => a.binType !== binTypeId) });
  }
  ...
  function toggleActive(id) {
    saveState({ residents: residents.map((r) => r.id === id ? { ...r, active: !r.active } : r) });
  }
  ...
  // ── Report full with name picker ─────────────────────────
  const [showReportPicker, setShowReportPicker] = useState(null); // binTypeId
    ...
    return (
      <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:"24px" }}>
        <div style={{ background:T.bgCard, borderRadius:"20px", padding:"24px", width:"100%", maxWidth:"320px", boxShadow:"0 20px 60px rgba(0,0,0,0.3)" }}>
          ...
          <div style={{ fontSize:"13px", color:T.textFaint, textAlign:"center", marginBottom:"16px" }}>Select your name to report this bin as full</div>
          <div style={{ display:"flex", flexDirection:"column", gap:"8px", maxHeight:"240px", overflowY:"auto" }}>
            {residents.filter(r => r.active).map((r) => {
              const alreadyReported = existing?.reports?.includes(r.name);
              return (
                <button key={r.id} className="btn" disabled={alreadyReported} onClick={() => { reportFull(binTypeId, r.name); setShowReportPicker(null); }}
                  style={{ padding:"12px 16px", fontSize:"15px", fontWeight:"500", background: alreadyReported?T.bgCard2:T.currentAccent, color: alreadyReported?T.textFaint:"#fff", border:`1px solid ${alreadyReported?T.border:T.currentAccent}`, textAlign:"left", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  {r.name}
                  {alreadyReported && <span style={{ fontSize:"12px" }}>Already reported ✓</span>}
                </button>
              );
            })}
          </div>
  ...
  function buildWAMessage(urgent) {
    ...
    if (urgent) {
      urgent.forEach((a) => {
        msg += `🚨 *URGENT: ${a.binLabel} is FULL!*\n`;
        msg += `Reported by: ${a.reports.join(", ")}\n\n`;
      });
    }
  ...
  function copyWAMessage(msg) {
    navigator.clipboard.writeText(msg).then(() => { setWaCopied(true); setTimeout(() => setWaCopied(false), 2500); }).catch(() => {});
  }
    ...
    <div style={{ minHeight:"100vh", background:T.bg, color:T.text, fontFamily:FONT, transition:"background 0.25s, color 0.25s" }}>
      ...
      <div style={{ background:T.bgCard, borderBottom:`1px solid ${T.border}`, padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:10 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
          ...
          {/* Dark/Light toggle */}
          <button className="btn" onClick={toggleTheme} style={{ background:T.bgCard2, border:`1px solid ${T.border}`, padding:"5px 10px", fontSize:"16px", lineHeight:1 }} title="Toggle dark/light mode">
            {isDark ? "☀️" : "🌙"}
          </button>
          {/* Admin badge */}
          {isAdmin && (
            <button className="btn" onClick={() => setIsAdmin(false)} style={{ background:T.adminBg, border:`1px solid ${T.adminBorder}`, color:T.adminText, padding:"4px 10px", fontSize:"11px", fontWeight:"700" }}>
              ADMIN ✕
            </button>
          )}
        </div>
      ...
      {/* URGENT ALERTS — shown when 2+ people report */}
            ...
            return (
              <div key={alert.id} className="urgent-pulse fade-in" style={{ background:T.urgentBg, border:`2px solid ${T.urgentBorder}`, borderRadius:"16px", padding:"16px", marginBottom:"10px" }}>
                ...
                <div style={{ display:"flex", gap:"8px" }}>
                  <button className="btn" onClick={() => { markEmptied(alert.binType); }} style={{ flex:1, background:T.currentAccent, color:"#fff", padding:"10px", fontSize:"13px", fontWeight:"700" }}>Mark Emptied ✓</button>
                  <button className="btn" onClick={() => copyWAMessage(urgentMsg)} style={{ flex:1, background:T.waGreen, color:"#fff", padding:"10px", fontSize:"13px", fontWeight:"700" }}>📋 Copy WhatsApp</button>
                  <a href={`https://wa.me/?text=${encodeURIComponent(urgentMsg)}`} target="_blank" rel="noreferrer" style={{ flex:1, background:T.waGreen, color:"#fff", padding:"10px", fontSize:"13px", fontWeight:"700", borderRadius:"10px", display:"flex", alignItems:"center", justifyContent:"center", textDecoration:"none" }}>Send WA ↗</a>
                </div>
              </div>
            );
          })}
      ...
      {/* Regular alerts (1 report only) */}
      {alerts.filter(a => !urgentAlerts.includes(a)).length > 0 && (
        <div style={{ padding:"12px 16px 0" }}>
            ...
            <div key={alert.id} className="alert-pulse fade-in" style={{ background:T.alertBg, border:`1.5px solid ${T.alertBorder}`, borderRadius:"14px", padding:"14px 16px", marginBottom:"10px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"12px" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:"13px", color:T.alertSubtext, marginTop:"2px" }}>By {alert.reports?.join(", ")} · {REPORTS_TO_URGENT - (alert.reports?.length||0)} more report triggers urgent alert</div>
                </div>
              </div>
              <div style={{ display:"flex", gap:"8px", flexShrink:0 }}>
                ...
                <button className="btn" onClick={() => dismissAlert(alert.binType)} style={{ background:T.bgCard2, color:T.textMuted, padding:"7px 10px", fontSize:"17px", lineHeight:1 }}>×</button>
              </div>
            </div>
          ))}
        </div>
            ...
            <div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
                {BIN_TYPES.map((bin) => {
                  ...
                  const isFull   = !!alert;
                  const isUrgent = isFull && Array.isArray(alert.reports) && alert.reports.length >= REPORTS_TO_URGENT;
                  if (isDone) return (
                  ...
                  return (
                    <button key={bin.id} className="btn" onClick={() => markEmptied(bin.id)} disabled={!currentPerson} style={{ width:"100%", borderRadius:"18px", border:"none", background:isUrgent?"linear-gradient(135deg,#ff6b00,#e65100)":isFull?"linear-gradient(135deg,#ff3b30,#c0392b)":isDark?"linear-gradient(135deg,#30d158,#28a745)":"linear-gradient(135deg,#34c759,#2dbe55)", display:"flex", alignItems:"center", overflow:"hidden", boxShadow:isFull?"0 4px 20px rgba(255,59,48,0.4)":"0 4px 20px rgba(52,199,89,0.35)" }}>
                      ...
                      <div style={{ flex:1, textAlign:"left", paddingLeft:"16px" }}>
                        <div style={{ fontSize:"11px", fontWeight:"600", color:"rgba(255,255,255,0.75)", textTransform:"uppercase", marginBottom:"2px" }}>{isUrgent?"🚨 URGENT — Tap to confirm emptied":isFull?"⚠️ Reported Full — Tap to confirm":"Tap when emptied"}</div>
                ...
                {BIN_TYPES.map((bin) => {
                  ...
                  return (
                    <div key={bin.id} style={{ background:isUrgent?T.urgentBg:isFull?T.alertBg:T.bgCard, border:`1.5px solid ${isUrgent?T.urgentBorder:isFull?T.alertBorder:T.border}`, borderRadius:"14px", padding:"14px", transition:"all 0.2s" }}>
                      ...
                      <div style={{ fontSize:"12px", color:isUrgent?T.urgentText:isFull?T.alertText:T.currentAccent, fontWeight:"600", marginBottom:"6px" }}>
                        {isUrgent?"🚨 URGENT":isFull?`⚠️ ${reportCount} report${reportCount>1?"s":""}`:"● OK"}
                      </div>
                      {!isFull
            ...
            <div style={{ background:T.waBg, border:`1.5px solid ${T.waBorder}`, borderRadius:"16px", padding:"16px 18px" }}>
              ...
              {showWA && (
                <div className="fade-in" style={{ marginTop:"14px" }}>
                  ...
                  <div style={{ display:"flex", gap:"8px" }}>
                    <button className="btn" onClick={() => copyWAMessage(buildWAMessage())} style={{ flex:1, background:waCopied?T.currentAccent:T.waGreen, color:"#fff", padding:"11px", fontSize:"14px", fontWeight:"700", transition:"background 0.3s" }}>{waCopied?"✅ Copied!":"📋 Copy"}</button>
